...
(function FullAppDisplay() {
  ...
  function displayUpdate() {
    let updateText = react.createElement(
      ...
      react.createElement(
        "li",
        {},
        "Added seekable progress bar: Now you can seek songs from FAD itself, click on it to seek!"
      ),
      react.createElement(
        "li",
        {},
        "Added show only on hover mode for volume bar (change in settings)"
      ),
      react.createElement(
        "li",
        {},
        "Bug Fixes: Reworked the upnext and queue function, to account for the scale."
      )
    );
  ...
  const VolumeBar = () => {
    ...
    useEffect(() => {
      ...
      return () =>
        Spicetify.Platform.PlaybackAPI._events.removeListener("volume", update);
    });
    return react.createElement(
      "div",
      {
        ...
        onWheel: (event) => {
          ...
          let temp =
            parseInt(document.querySelector("#fad-volbar-inner").style.height) /
              2 +
            dir * 1;
          if (temp < 0) {
            temp = 0;
          ...
          document.querySelector("#fad-volbar-inner").style.height = `${
            2 * temp
          }px`;
        },
      },
        ...
        react.createElement(ButtonIcon, {
          ...
          onClick: () => {
            if (!Spicetify.Player.getMute()) {
              ...
              setTimeout(() => {
                document.querySelector("#fad-volbar-inner").style.height = `${
                  Spicetify.Player.getVolume() * 200
                }px`;
              }, 200);
            }
          },
  ...
  const upNext = async ({ index, queue }) => {
    ...
    if (coor.bottom + scale * 90 + 10 > window.innerHeight) {
      bottom =
        bottom +
        (coor.bottom + 90 * scale + 10 - window.innerHeight) / scale +
        10;
      right = -409;
    }
    ...
    if (Spicetify.Queue.nextTracks[index].provider == "context") {
      ...
      if (!context) {
        const uriObj = Spicetify.URI.fromString(
  ...
  const PlayerControls = () => {
    ...
    const [isPodcast, setPodcast] = useState(
      Spicetify.URI.isEpisode(Spicetify.Player.data.track.uri)
    );
    useEffect(() => {
      const update = ({ data }) => {
        data.track.metadata["collection.in_collection"] == "true"
          ? setHeart(true)
          : setHeart(false);
        ...
        if (
          !state.restrictions?.canToggleRepeatContext ||
          !state.restrictions?.canToggleRepeatTrack
        ) {
          setRepeat(undefined);
        }
      };
    ...
    return react.createElement(
      ...
      react.createElement(ButtonIcon, {
        ...
        className: isShuffle
          ? "dot-after"
          : // @ts-ignore
          !Spicetify.Player.origin._state.restrictions?.canToggleShuffle ||
            isShuffle == undefined
          ? "crossed-out"
          : "",
        // style: {
      ...
      react.createElement(ButtonIcon, {
        ...
        className: isRepeat
          ? "dot-after"
          : // @ts-ignore
          !Spicetify.Player.origin._state.restrictions
      ...
      CONFIG.enableControl &&
        react.createElement(ButtonIcon, {
          ...
          onClick: async (e) => {
            ...
            notitext.className =
              "main-notificationBubble-NotificationBubble main-notificationBubble-isNotice";
              ...
              "div",
              {
                ...
                onWheel: (e) => {
                  ...
                  if (e.deltaY > 0) {
                    ...
                    CONFIG["viewing"] += 1;
                    var item =
                      document.querySelector("#scroll-queue").childNodes[
                        CONFIG["viewing"]
                      ];
                    // @ts-ignore
  ...
  class FAD extends react.Component {
    ...
    async fetchInfo() {
      ...
      let isLocalOrEpisode =
        Spicetify.URI.isLocalTrack(Spicetify.Player.data.track.uri) ||
        Spicetify.URI.isEpisode(Spicetify.Player.data.track.uri);

      if (!isLocalOrEpisode) {
        const ximage = await Spicetify.CosmosAsync.get(
          "https://api.spotify.com/v1/tracks/" + uriFinal
        );
        ...
        style.innerHTML =
          ...
          (window.innerHeight > window.innerWidth && CONFIG.verticalMonitor
            ? verticalMonitorStyle
            : "");
      }
    ...
    async animateCanvasColor(prevUri, nextUri) {
      ...
      if (CONFIG.enableGrad) {
        ...
        if (colorsarr.length < 3) {
          ...
          prevColor.addColorStop(
            1,
            colorsarr[2] ?? colorsarr[1] ?? colorsarr[0]
          );
        }
          ...
          react.createElement(
            ...
            react.createElement(
              ...
              react.createElement(
                ...
                CONFIG.enableExtraControl &&
                  react.createElement(ExtraPlayerControls),
                CONFIG.enableProgress && react.createElement(ProgressBar)
              )
            )
          ),
          // @ts-ignore
          !(CONFIG["volumeBar"] === "disable") &&
  ...
  function autoHideLyrics() {
    ...
    if (!document.querySelector("#fad-lyrics-plus-container").innerText) {
      ...
      // @ts-ignore
      if (
        document.querySelector("#fad-lyrics-plus-container").innerText ==
        "(• _ • )"
      ) {
        isHidden = true;
        updateStyle();
  ...
  `,
    ...
    let configContainer = react.createElement(
      ...
      react.createElement(ConfigItem, {
        name: "Enable extra controls",
        field: "enableExtraControl",
        func: updateVisual,
      }),
      react.createElement(ConfigSelection, {
        name: "Trim title",
      ...
      CONFIG.lyricsPlus &&
        ...
        react.createElement(ConfigItem, {
          name: "Vertical Monitor Mode",
          field: "verticalMonitor",
          func: updateStyle,
        }),
      react.createElement(ConfigItem, {
  ...
  // on clicking f7 set toggleFullscreen to true , if its already true then set it to false
  Spicetify.Mousetrap.bind("f7", () => {
    CONFIG["enableFullscreen"] = !CONFIG["enableFullscreen"];
    saveConfig();
    toggleFullscreen();
  });
})();
